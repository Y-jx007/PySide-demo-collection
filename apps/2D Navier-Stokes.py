...
default_D = 20
default_L = 20
default_nu = default_u0 * default_L / default_re
...
inv_tau = ti.field(ti.f32, shape=())
u0       = ti.field(ti.f32, shape=())
D        = ti.field(ti.f32, shape=())
L        = ti.field(ti.f32, shape=())
...
class CustomObstacleCompiler:
    def __init__(self):
        self.globals_dict = {
            'math': math, 'np': np, 'ti': ti,
            'sin': math.sin, 'cos': math.cos, 'exp': math.exp,
            'sqrt': math.sqrt, 'abs': abs, 'pi': math.pi, 'e': math.e
        }

    def compile_obstacle(self, code_str, width, height, cx, cy, D_val):
        try:
            func_code = f"""
def obstacle_func(x, y, width={width}, height={height}, cx={cx}, cy={cy}, D={D_val}):
    {code_str}
    return result
"""
            exec(func_code, self.globals_dict)
            obstacle_func = self.globals_dict['obstacle_func']
            mask_arr = np.zeros((width, height), dtype=np.float32)
            ...
            for i in range(width):
                for j in range(height):
                    try:
                        val = obstacle_func(X[i, j], Y[i, j])
                        mask_arr[i, j] = 1.0 if val else 0.0
                    except:
                        mask_arr[i, j] = 0.0
...
obstacle_compiler = CustomObstacleCompiler()
...
@ti.kernel
def update_inv_tau():
    nu_val = u0[None] * L[None] / Re[None]
    nu[None] = nu_val
    tau = 3.0 * nu_val + 0.5
    if tau < 0.5001:
        tau = 0.5001
    inv_tau[None] = 1.0 / tau
...
def init_fields():
    for i, j in ti.ndrange(nx, ny):
        ...
        f_new[i, j] = f_old[i, j]
        mask[i, j] = 0.0
    disturb_start = int(nx * 0.15)
    disturb_end = int(nx * 0.19)
    for i, j in ti.ndrange(nx, ny):
        if mask[i, j] == 0.0 and i > disturb_start and i < disturb_end:
...
def collide_and_stream():
    for i, j in ti.ndrange((1, nx-1), (1, ny-1)):
        for k in ti.static(range(9)):
            ...
            jp = j - e[k, 1]
            feq_k = f_eq(ip, jp)[k]
            f_new[i, j][k] = (1.0 - inv_tau[None]) * f_old[ip, jp][k] + feq_k * inv_tau[None]
...
def apply_bc():
    ...
    for i, j in ti.ndrange(nx, ny):
        if mask[i, j] == 1.0:
            vel[i, j] = ti.Vector([0.0, 0.0])
            inb = i + 1 if i < cylinder_cx else i - 1
            jnb = j + 1 if j < cylinder_cy else j - 1
            apply_bc_core(0, 0, i, j, inb, jnb)
...
def init_color_luts():
    for i in range(lut_size):
        t = i / (lut_size - 1)
        if t < 0.5:
            vort_lut[i] = ti.Vector([t*2, t*2, 1.0])
        else:
            vort_lut[i] = ti.Vector([1.0, 2*(1-t), 2*(1-t)])
    for i in range(lut_size):
        t = i / (lut_size - 1)
        if t < 0.33:
            speed_lut[i] = ti.Vector([0.0, 0.0, t*3])
        elif t < 0.66:
            speed_lut[i] = ti.Vector([(t-0.33)*3, 0.0, 1.0])
        else:
            speed_lut[i] = ti.Vector([1.0, (t-0.66)*3, 1.0-(t-0.66)*3])
...
class ControlPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self.custom_obstacle_code = "result = (x-cx)**2 + (y-cy)**2 < (D/2)**2"
    ...
    def _init_ui(self):
        ...
        self.nu_slider = QSlider(Qt.Horizontal); self.nu_slider.setRange(1,500)  # 0.0005~0.25
        self.nu_slider.setValue(int(default_nu*2000))
        ...
        self.u_slider = QSlider(Qt.Horizontal); self.u_slider.setRange(1,30)   # 0.01~0.30
        self.u_slider.setValue(int(default_u0*100))
        ...
        self.l_slider = QSlider(Qt.Horizontal); self.l_slider.setRange(5,80); self.l_slider.setValue(default_L)
        self.l_label = QLabel(str(default_L))
        g.addWidget(QLabel("L"),3,0); g.addWidget(self.l_slider,3,1); g.addWidget(self.l_label,3,2)
        self.l_slider.valueChanged.connect(self._on_l_changed)

        self.tau_label = QLabel("0.5001")
        g.addWidget(QLabel("τ"),4,0); g.addWidget(self.tau_label,4,1,1,2)
        ...
        obs = QGroupBox("障碍物")
        ol = QVBoxLayout(obs)
        self.obs_combo = QComboBox(); self.obs_combo.addItems(["圆柱","方柱","单缝","双缝","自定义"])
        self.obs_combo.currentIndexChanged.connect(self._on_obs_preset_changed)
        ol.addWidget(self.obs_combo)
        ol.addWidget(QLabel("自定义表达式:"))
        self.code_editor = QTextEdit(); self.code_editor.setPlainText(self.custom_obstacle_code)
        self.code_editor.setMinimumHeight(100)
        ol.addWidget(self.code_editor)
        btn = QHBoxLayout()
        self.apply_obs_btn = QPushButton("应用"); self.reset_obs_btn = QPushButton("重置")
        btn.addWidget(self.apply_obs_btn); btn.addWidget(self.reset_obs_btn)
        ol.addLayout(btn)
        left.addWidget(obs)
        ...
        self.d_slider.valueChanged.connect(lambda v: (self.d_label.setText(str(v)), self._reapply_current_obstacle()))
        right.addWidget(geo)
        ...
        self.apply_obs_btn.clicked.connect(self.apply_custom_obstacle)
        self.reset_obs_btn.clicked.connect(self.reset_obstacle_code)
    ...
    def _sync_re_nu(self, changed):
        ...
        if changed == 're':
            ...
            if nu_slider_val < 1:
                ...
                # 根据新的nu反算Re
                new_re = int(u * L_val / nu_val)
                new_re = max(10, min(500, new_re))
                self.re_slider.setValue(new_re)
                self.re_label.setText(str(new_re))
            elif nu_slider_val > 500:
                nu_slider_val = 500
                nu_val = nu_slider_val / 2000.0
                new_re = int(u * L_val / nu_val)
                new_re = max(10, min(500, new_re))
                self.re_slider.setValue(new_re)
                self.re_label.setText(str(new_re))
            self.nu_slider.setValue(nu_slider_val)
            self.nu_label.setText(f"{nu_val:.4f}")
        elif changed == 'nu':
            nu_val = self.nu_slider.value() / 2000.0
            if nu_val < 0.0005: nu_val = 0.0005
            re = int(u * L_val / nu_val)
            re = max(10, min(500, re))
            self.re_slider.setValue(re)
            self.re_label.setText(str(re))
            self.nu_slider.setValue(int(nu_val*2000))
            self.nu_label.setText(f"{nu_val:.4f}")
        else:  # u 或 L 改变，保持Re不变更新ν
            re = self.re_slider.value()
            nu_val = u * L_val / re
            nu_slider_val = int(nu_val * 2000)
            if nu_slider_val < 1:
    ...
    def _on_l_changed(self, v): self.l_label.setText(str(v)); self._sync_re_nu('l')

    def _update_tau_display(self):
        u = self.u_slider.value()/100.0; L_val = self.l_slider.value(); re = self.re_slider.value()
        nu_val = u * L_val / re; tau = 3.0*nu_val + 0.5
    ...
    def apply_custom_obstacle(self):
        code = self.code_editor.toPlainText(); D_val = float(self.d_slider.value())
        mask_arr, error = obstacle_compiler.compile_obstacle(code, nx, ny, cylinder_cx, cylinder_cy, D_val)
        if error: QMessageBox.warning(self,"编译错误",f"自定义障碍物编译失败:\n{error}"); return
        update_mask_from_array(mask_arr); self.obs_combo.setCurrentIndex(4)

    def reset_obstacle_code(self): self.code_editor.setPlainText(self.custom_obstacle_code); self.obs_combo.setCurrentIndex(0); self.apply_cylindrical_obstacle()
    def toggle_sim(self): self.simulation_running = not self.simulation_running; self.pause_btn.setText("暂停" if self.simulation_running else "开始")
    def reset_sim(self):
        self.apply_params(); update_inv_tau(); init_fields()
        idx = self.obs_combo.currentIndex()
        if idx==0: self.apply_cylindrical_obstacle()
        elif idx==1: self.apply_square_obstacle()
        elif idx==2: self.apply_single_slit()
        elif idx==3: self.apply_double_slit()
        elif idx==4: self.apply_custom_obstacle()
        calc_vorticity(); calc_speed()

    def apply_params(self):
        Re[None] = float(self.re_slider.value()); u0[None] = self.u_slider.value()/100.0
        L[None] = float(self.l_slider.value()); D[None] = float(self.d_slider.value())
        init_constants()

    def get_steps(self): return self.steps_slider.value()

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("卡门涡街 800×250 完整参数"); self.setGeometry(100,100,1280,560)
        ...
        self.control.apply_params(); L[None]=default_L; update_inv_tau(); init_fields()
    ...
    def _update_sim(self):
        if self.control.simulation_running:
            self.control.apply_params(); update_inv_tau()
            for _ in range(self.control.get_steps()): collide_and_stream(); update_macro(); apply_bc()
            calc_vorticity(); calc_speed(); visualize_vort(); visualize_speed()
            self.vort_display.update(); self.speed_display.update()

    def closeEvent(self, event): self.timer.stop(); ti.reset(); event.accept()
